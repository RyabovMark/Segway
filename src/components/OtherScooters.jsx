...
const OtherScooters = ({displayedElementId}) => {
  ...
  const nameWithoutBrand = (str, subStr) => {
    ...
    return str.includes(subStr) ? str.substring(position + subStr.length, str.length) : str
  }
  ...
  const newArray = () => {
    ...
    while (true) {
      ...
      if (displayedElementId != randomNumber) otherScootersSet.add(randomElement)
  ...
  return (
    <Box>
      <Divider/>
      <Typography textAlign='center' color='text.primary' fontWeight={600}
                  textTransform='uppercase' fontSize='27px'>Other
        models</Typography>
      <Typography textAlign='center' color='text.primary'
                  textTransform='capitalize' fontSize='18px'>Check out the
        entire line of Segway
        scooters</Typography>
      <Box sx={{width: '100%'}}>
            ...
            <Grid item key={item.id} xs={4}>
              <Box display='flex' flexDirection='column' alignItems='center'
                ...
                <Typography fontSize='16px'
                            ...
                            fontWeight={600}>{nameWithoutBrand(item.name, brandName)}</Typography>
                <Typography fontSize='13px' fontWeight={600}
                            color='text.primary'>${item.cost}</Typography>
                <Link to={`/catalog/${item.id}`}>
                  <Button onClick={() => handlerScrollToTop()}
                          variant='outlined'
                          size='small'
                          sx={{
