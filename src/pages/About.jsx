    ...
    <Box sx={{flex: '1 1 auto'}}>
      <Box display='flex' flexDirection='column' width='90%' mx='auto'>
        <Typography textTransform='uppercase' textAlign='center'
                    fontWeight={700} color='text.primary' my='30px'
                    sx={{fontSize: {xs: '25px', md: '50px'}}}>About
          us</Typography>
        <Box display='flex' mx='-20px'
          ...
          <Box mx='20px' sx={{width: {xs: '35%', md: '35%'}}}>
            ...
            >we are SegWay</Typography>
            <Typography color='text.primary' textAlign='right'
                        xs={{fontSize: {xs: '10px', md: '20px'}}}>Tough
              EVA material, suitable for the entire Ninebot
              KickScooter by Segway line. 3L capacity gives you plenty of room
              to carry your phone, charger, bottled water, and other personal
              items.</Typography>
          </Box>
          <Box position='relative' mx='20px'
               sx={{
                 display: 'flex',
            ...
            {aboutUs.map(item => (
              <Grid item key={item.id} xs={6} md={4}>
                <Box display='flex' flexDirection='column' alignItems='center'
                  ...
                  <Typography color='text.primary' mb='10px'
                              fontSize='25px'>{item.nane}</Typography>
                  <Typography color='text.secondary'
                              textTransform='capitalize'
                              fontSize='15px'>{item.job}</Typography>
                  <Typography color='text.secondary' my='5px'
                              fontSize='15px'><CallOutlinedIcon/> {item.phone}
                  </Typography>
                  <Typography color='text.secondary'
                              fontSize='15px'><EmailOutlinedIcon/> {item.mail}
                  </Typography>
                </Box>
              </Grid>
            ))}
        ...
        <Box sx={{mb: '30px'}}>
          ...
          >Latest Review</Typography>
          <Box>
            {reviewsData.map(item => (
              <TabPanel key={item.id} value={tabsReview} index={item.id}>
                <Box display='flex' flexDirection='column' p='20px' borderRadius='20px'
                     bgcolor='secondary.greyOpacity'>
                  <Box display='flex' flexDirection='row' alignItems='center'
                       justifyContent='flex-start' mb='20px'>
                    <Avatar src={item.avatar}
                            sx={{width: '70px', height: '70px', mr: '20px'}}/>
                    <Typography color='text.primary' fontWeight={700}
                                fontSize='25px'>{item.writer}</Typography>
                  </Box>
                  <Box height='140px' overflow='hidden' mb='10px'>
                  <Typography color='text.primary' textAlign='justify' fontSize='15px'>{item.review}</Typography>
                  </Box>
                  <Typography color='text.primary' fontSize='15px' fontStyle='italic'>{item.date}</Typography>
                </Box>
              </TabPanel>
            ))}
          </Box>
          <Tabs value={tabsReview}
